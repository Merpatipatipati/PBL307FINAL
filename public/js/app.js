...
// Ganti import axios dengan require
var axios = __webpack_require__(/*! axios */ "./node_modules/axios/dist/browser/axios.cjs");
window.axios = axios;
window.axios.defaults.headers.common['X-Requested-With'] = 'XMLHttpRequest';

/**
 * Mengimpor Echo dan socket.io-client untuk real-time events
 */
// Ganti import Echo dan socketio dengan require
...
// Pastikan conversationId dan authUserId sudah didefinisikan di Blade
var conversationId = window.conversationId;
var authUserId = window.authUserId;
console.log(window);

// Menyiapkan Echo untuk mendengarkan channel real-time
window.Echo = new Echo({
  broadcaster: 'socket.io',
  host: 'http://192.168.136.10:6001',
  // Sesuaikan dengan IP server Laravel Echo
  client: socketio
});

// Menambahkan listener pada channel 'conversation.{conversationId}'
window.Echo.channel('conversation.' + conversationId).listen('MessageSent', function (event) {
  console.log(event.message);
  // Menampilkan pesan baru di chat secara real-time
  var messageBox = document.getElementById('messages-box');
  var messageElement = document.createElement('div');
  messageElement.classList.add('message-item', 'mb-3');
  messageElement.innerHTML = "\n            <div class=\"".concat(event.message.sender_id === authUserId ? 'text-right' : 'text-left', "\">\n                <p class=\"mb-1\"><strong>").concat(event.message.sender.username, "</strong></p>\n                <p class=\"bg-light p-2 rounded d-inline-block\" style=\"background-color: ").concat(event.message.sender_id === authUserId ? '#007bff' : '#f8f9fa', "; color: black;\">\n                    ").concat(event.message.content, "\n                </p>\n                <small class=\"text-muted d-block\">").concat(event.message.sent_at, "</small>\n            </div>\n        ");
  messageBox.appendChild(messageElement);
  messageBox.scrollTop = messageBox.scrollHeight; // Scroll ke bawah untuk menampilkan pesan terbaru
});
